    ...
    if (APPLE)
        ...
        if (IS_CROSS_COMPILE)
            ...
            if (CMAKE_CROSSCOMPILING AND CMAKE_SYSTEM_NAME STREQUAL "iOS")
                set(_gmp_ios_target_arch "${_gmp_host_arch}")
                if (_gmp_ios_target_arch STREQUAL "aarch64")
                    set(_gmp_ios_target_arch "arm64")
                endif ()
                set(_gmp_ccflags "${_gmp_ccflags} ${_gmp_host_arch_flags} -miphoneos-version-min=${DEP_OSX_TARGET} -target ${_gmp_ios_target_arch}-apple-ios${DEP_OSX_TARGET} -isysroot ${CMAKE_OSX_SYSROOT}")
            else ()
                set(_gmp_ccflags "${_gmp_ccflags} ${_gmp_host_arch_flags} -mmacosx-version-min=${DEP_OSX_TARGET}")
            endif ()
            if (CMAKE_CROSSCOMPILING)
                # For true cross-compiles (for example iOS), pass --host only
                # via TOOLCHAIN_PREFIX to avoid duplicate/conflicting --host args.
