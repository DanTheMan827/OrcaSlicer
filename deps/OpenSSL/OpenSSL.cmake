...
if(WIN32)
    ...
    if(APPLE)
        if (CMAKE_SYSTEM_NAME STREQUAL "iOS")
            set(_ios_target_flag "-target ${CMAKE_OSX_ARCHITECTURES}-apple-ios${CMAKE_OSX_DEPLOYMENT_TARGET}")
            set(_ios_sysroot_flag "-isysroot ${CMAKE_OSX_SYSROOT}")
            set(_ios_minver_flag "-miphoneos-version-min=${CMAKE_OSX_DEPLOYMENT_TARGET}")
            set(_conf_cmd env "CC=${CMAKE_C_COMPILER}" "CFLAGS=${_ios_target_flag} ${_ios_sysroot_flag} ${_ios_minver_flag}" "LDFLAGS=${_ios_target_flag} ${_ios_sysroot_flag} ${_ios_minver_flag}" ./Configure)
        else()
    ...
    if (CMAKE_CROSSCOMPILING)
        if (CMAKE_SYSTEM_NAME STREQUAL "iOS")
            # iOS builds use clang + explicit target/sysroot flags above.
            set(_cross_comp_prefix_line "")
        else()
